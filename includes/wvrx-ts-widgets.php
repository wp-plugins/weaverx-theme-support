...
class WeaverX_Widget_Text extends WP_Widget {
	...
	function WeaverX_Widget_Text() {
		...
		$this->WP_Widget('wvrx2_text', 'Weaver X Text 2', $widget_ops, $control_ops);
	}
...
class WeaverX_Widget_PPText extends WP_Widget {
	...
	$widget_ops = array('classname' => 'wvrx_widget_pptext', 'description' =>
	    'Display text on a Per Page basis. Add to Widget area to see instructions.' );
	$this->WP_Widget('wvrx_pptext', 'Weaver X Per Page Text', $widget_ops);
    }
...
class WeaverX_Widget_Login extends WP_Widget {
	...
	function WeaverX_Widget_Login() {
		$widget_ops = array('classname' => 'wvrx_widget_login', 'description' => __( "Log in/out, admin", 'wvrx_ts' ) );
		$this->WP_Widget('wvrx_login', 'Weaver X Login', $widget_ops);
	}

	function widget( $args, $instance ) {
		extract($args);
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Login', 'wvrx_ts' ) : $instance['title'], $instance, $this->id_base);
		...
		echo '<span class="wvrx-welcome-user">' . __('Welcome','wvrx_ts') . ' ' . $current_user->display_name . ".</span><br />\n";
?>
		<ul>
		<?php wp_register(); ?>
